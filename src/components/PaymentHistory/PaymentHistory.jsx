...
const PaymentHistory = () => {
    ...
    return (
        <div className='w-1/2 bg-white rounded-[20px] custom-shadow text-interactive-dark-hover flex flex-col justify-between mx-auto'>
            ...
            {history.map(item => <div className="grid grid-cols-4 p-4 text-center text-interactive-light-focus" key={item._id}>
                <p className='text-wrap'>{item.mail.split("@")[0]}</p>
                <p className='text-wrap'>&#8377; {item.amount}</p>
                <p>{item.disbursed ? 'Disbursed' : item.declined ? 'Declined' : ""}</p>
