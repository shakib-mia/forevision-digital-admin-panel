...
const AppleMusicProfileRelocate = () => {
  ...
  const handleApprove = () => {
    ...
    axios
      .put(backendUrl + "submit-form/apple-music-fresh-profile/" + _id, rest)
      .then(() => {
        Swal.fire({
          icon: "success",
          title: "Approved",
          text: "The Apple Music Fresh Profile Request has been approved.",
        });
        setRefetch(!refetch);
      ...
      axios
        ...
        .then(() => {
          Swal.fire({
            ...
            text: "The Apple Music Fresh Profile Request has been denied.",
          });
          setRefetch(!refetch);
          setDetailsId(-1); // Close the modal
        })
      ...
      <table className="w-full">
        ...
        <tbody>
          {data.map((item, id) => (
            ...
            >
              <td className="py-2">
                {item.apple_music_fresh_profile_name ||
                  item.apple_music_fresh_profile_artist_name}
              </td>
              <td className="py-2">
                {" "}
                {item.apple_music_fresh_profile_email || item.emailId}
              </td>
          ...
          <table className="text-black border w-full divide-y border-grey rounded">
            {Object.keys(data[detailsId]).map((item) => {
              if (item !== "_id") {
                ...
                const fieldName = item.startsWith("apple_music_fresh_profile_")
                  ? item
                      .replace("apple_music_fresh_profile_", "")
